...
# Click show menu
if not 'Acerca de este tablero' in driver.page_source:
...
delay_until_show_text(driver, "Enviar al tablero")

els_archivados = driver.find_elements(By.CLASS_NAME, 'archived-list-card')
cantidad_els_antes = len(els_archivados)

sleep(1)

# Desarchivar
driver.find_element(By.CSS_SELECTOR, '#content > div > div.board-menu.js-fill-board-menu > div > div > div.board-menu-content.u-fancy-scrollbar.js-board-menu-content-wrapper > div > div > div.archive-content.js-archive-content > div > div > div > div:nth-child(1) > div > div > p > a.js-reopen')\
  .click()

sleep(1)

els_archivados = driver.find_elements(By.CLASS_NAME, 'archived-list-card')
cantidad_els_despues = len(els_archivados)

if cantidad_els_antes - cantidad_els_despues == 1:
  print("Test Exitoso")
else:
  print("Test Fallo")